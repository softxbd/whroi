            ...
            <div class="col-lg-12 col-md-12 mb-4">
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h5 class="card-title mb-0">ADD GET INVOLVED</h5>
                    </div>
                    <div class="card-body pb-2">
                        <form class="mb-3" action="{{ route('store.involved') }}" method="post">
                            @csrf
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="mb-3">
                                        <label class="form-label">CHOOSE GET INVOLVED CATEGORY<span class="text-danger">*</span></label>
                                        <select name="get_involved_category_id" class="form-control select2" required>
                                            @foreach($get_involved_categories as $get_involved_category)
                                            <option value="{{ $get_involved_category->id }}">{{ $get_involved_category->name }}</option>
                                            @endforeach
                                        </select>
